...
const REVIEWER_QUOTE_KEYS = ['nav.quotes.view', 'quotes.view', 'quotes.create', 'quotes.update'];
const CONSULTA_QUOTE_KEYS = ['nav.quotes.view', 'quotes.view'];
...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    for (const r of reviewers) {
      for (const key of REVIEWER_QUOTE_KEYS) {
        const pid = keyToId[key];
        if (pid) {
          bulk.push({
            role_id: r.id,
    ...
    for (const r of consultas) {
      for (const key of CONSULTA_QUOTE_KEYS) {
        const pid = keyToId[key];
        if (pid) {
          bulk.push({
            role_id: r.id,
