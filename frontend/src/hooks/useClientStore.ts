...
const mapAssignedUser = (u: any) => (u ? { id: u.id, name: u.name, email: u.email } : null);
const mapProduct = (p: any) => (p ? { id: p.id, title: p.title } : null);
...
const mapClient = (c: any): Client => ({
  ...
  notes: c.notes,
  status: (c.status as Client['status']) || 'pending',
  ...
  totalAmountDue:
    c.total_amount_due != null ? Number(c.total_amount_due) : c.totalAmountDue != null ? Number(c.totalAmountDue) : undefined,
...
export const useClientStore = () => {
  ...
  const fetchClients = useCallback(
    async (token: string, params?: ClientQueryParams) => {
      setIsLoading(true);
      try {
        ...
        const normalizedResponse = Array.isArray(response)
          ? { data: response, meta: undefined, templates: [] }
          : response || {};
        ...
        const mapped: Client[] = clientsData.map((c: any) => mapClient(c));
        setClients(mapped);
  ...
  const fetchClientsForPickers = useCallback(async (token: string, opts?: { assignedUserId?: string }) => {
    try {
      const merged: Client[] = [];
      ...
      do {
        ...
        const normalizedResponse = Array.isArray(response) ? { data: response, meta: undefined } : response || {};
        const clientsData = Array.isArray(normalizedResponse.data) ? normalizedResponse.data : [];
  ...
  const createClient = useCallback(
    async (token: string, clientData: Omit<Client, 'id' | 'createdAt' | 'updatedAt' | 'formsCompleted'>) => {
      try {
    ...
    try {
      ...
      const mergeLists = (prev: Client[]) => {
        ...
        if (oldPid !== newPid) {
          if (oldPid) {
            next = next.map((c) =>
              c.id === oldPid ? { ...c, children: (c.children ?? []).filter((x) => x.id !== clientId) } : c
            );
          }
          if (newPid) {
            next = next.map((c) => {
  ...
  const getClientStats = useCallback(
    async (token: string, params?: { assignedUserId?: string }) => {
      try {
